...
/*
 ...
 @param num1 Numero a introducir (1)
 @param num2 Numero a introducir (2)
 @param MCD Resultado final(MCD)
 @return Resultado final(mCM)
 @pre ninguna
 @post ninguna
 @autor Pablo
 @version 1.0 FP DAM
 */
int CalculamCM(int, int, int);
...
int CalculamCM (int MCD, int a, int b) {
	return ((a * b) / MCD);
}

void Resultado (int resultado1, int resultado2) {

	if (resultado1 == 1){
		cout << "Los numeros introducidos no tienen MCD (default => 1)\n";
	}else{
		cout << "El Máximo Común Divisor es: " << resultado1 << endl;
	}
	cout << "El Mínimo Común Múltiplo es: " << resultado2 << endl;
}
